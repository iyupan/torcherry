...
@pipeline_def
def create_dali_cifar10_tfrec_pipeline(data_path, data_index_path, type, num_shards, shard_id, crop=28, dali_cpu=False):
...
def get_cifar10_iter_dali(type, image_dir, batch_size, num_threads, seed, dali_cpu, gpu_num, auto_reset=True):
    ...
    data_path = os.path.join(image_dir, "CIFAR10_tfrecords", "processed")

    if type == 'train':
        train_image_path=os.path.join(data_path, "cifar10_train.tfrecords")
        train_image_index_path=os.path.join(data_path, "cifar10_train_idx")

        pipes = []
        for i in range(gpu_num):
            pipe = create_dali_cifar10_tfrec_pipeline(data_path=train_image_path,
        ...
        val_image_path = os.path.join(data_path, "cifar10_test.tfrecords")
        val_image_index_path = os.path.join(data_path, "cifar10_test_idx")

        pipes = []
        for i in range(gpu_num):
            pipe = create_dali_cifar10_tfrec_pipeline(data_path=val_image_path,
